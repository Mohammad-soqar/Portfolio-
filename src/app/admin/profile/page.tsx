...
export default function ProfileAdmin() {
  ...
  const handleImageUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert(`Upload failed: ${err.message}. Check if Storage is enabled and rules allow writes.`);
    } finally {
      setUploading(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="profile-form">
            ...
            <div className="current-pic">
              {formData.profilePic ? (
                <div className="relative w-full h-full">
                  <Image src={formData.profilePic} fill className="object-cover" alt="Profile" />
                </div>
              ) : (
                <div className="no-pic"><User size={48} /></div>
              )}
              {uploading && <div className="upload-overlay">Uploading...</div>}
            </div>
            <label className="upload-btn">
              <Upload size={16} /> {formData.profilePic ? "Change Photo" : "Upload Photo"}
        ...
        <div className="form-grid">
          <div className="form-section glass">
            <h3><Briefcase size={20} /> Contact & Presence</h3>
            ...
            <div className="input-field">
              ...
              <input value={formData.cvLink} onChange={e => setFormData({...formData, cvLink: e.target.value})} />
            </div>
          </div>

          <div className="form-section glass">
            <h3><Award size={20} /> Key Metrics</h3>
            <div className="input-field">
              <label><Github size={14} /> GitHub Contributions</label>
              <input placeholder="e.g. 2k+" value={formData.githubContributions} onChange={e => setFormData({...formData, githubContributions: e.target.value})} />
            </div>
            <div className="input-field">
              <label>Years of Experience</label>
              <input placeholder="e.g. 2+" value={formData.experienceYears} onChange={e => setFormData({...formData, experienceYears: e.target.value})} />
            </div>
            <div className="input-field">
              <label>Projects Completed</label>
              <input placeholder="e.g. 14+" value={formData.projectsDone} onChange={e => setFormData({...formData, projectsDone: e.target.value})} />
            </div>
          </div>
        </div>
...
استخدم # للعناوين، - للقوائم، **عريض** للتأكيد."
      ...
      <style jsx>{`
        ...
        .no-pic { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; opacity: 0.3; }
        .upload-overlay { position: absolute; inset: 0; background: rgba(0,0,0,0.7); display: flex; align-items: center; justify-content: center; font-size: 0.8rem; }
        ...
        .upload-btn:hover { background: rgba(118, 40, 229, 0.2); }

        .basic-info { flex: 1; display: grid; gap: 1.5rem; }
        ...
        .form-section h3 { margin-bottom: 1.5rem; display: flex; align-items: center; gap: 10px; font-size: 1.1rem; }
        .full-width { grid-column: 1 / -1; }

        .input-field { display: flex; flex-direction: column; gap: 0.6rem; margin-bottom: 1.2rem; }
        label { font-size: 0.8rem; font-weight: 600; opacity: 0.5; text-transform: uppercase; letter-spacing: 0.05em; display: flex; align-items: center; gap: 6px; }
        input, textarea {
        ...
        input:focus, textarea:focus { border-color: #7628E5; outline: none; }

        .section-header-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem; }
        .preview-tabs { display: flex; gap: 4px; background: rgba(0,0,0,0.2); padding: 4px; border-radius: 8px; }
        .preview-tabs button { display: flex; align-items: center; gap: 8px; border: none; background: transparent; color: rgba(255,255,255,0.4); padding: 8px 16px; border-radius: 6px; cursor: pointer; font-size: 0.8rem; font-weight: 600; transition: 0.3s; }
        .preview-tabs button.active { background: #7628E5; color: white; }
        ...
        .markdown-preview :global(h1), .markdown-preview :global(h2), .markdown-preview :global(h3) { color: white; margin: 1.5rem 0 1rem; }
        .markdown-preview :global(ul), .markdown-preview :global(ol) { padding-left: 1.5rem; margin-bottom: 1rem; }
        .markdown-preview :global(li) { margin-bottom: 0.5rem; }
        .markdown-preview :global(strong) { color: white; font-weight: 700; }
        ...
        .save-btn:hover { background: #8a3ffc; transform: translateY(-2px); box-shadow: 0 10px 30px rgba(118, 40, 229, 0.4); }

        @media (max-width: 768px) {
          .profile-top-card { flex-direction: column; padding: 1.5rem; }
